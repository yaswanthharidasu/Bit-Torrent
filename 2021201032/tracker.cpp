...
public:
    ...
    // ================================== Utitlity methods ========================================
    void chunk_info(string group_name, string file_name, string username, int desc);
...
void Tracker::trackerAsServer(int desc) {
    while (true) {
        ...
        if (words[0] == GET_CHUNK_INFO) {
            chunk_info(words[1], words[2], words[3], desc);
            break;
        }
...
void Tracker::upload_file(string file_path, string group_name, string username, int desc) {
    ...
    if (allPeers.find(username) == allPeers.end() || allPeers[username].loggedIn == false) {
        ...
        if (allGroups[group_name].files.find(file_name) != allGroups[group_name].files.end()) {
            allGroups[group_name].files[file_name].users.push_back(username);
            ...
            allGroups[group_name].files[file_name] = newFile;
            reply = UPLOAD_FILE_SUCCESS;
            log.printLog("Uploaded File: " + file_name + " in the group: " + group_name + " by the user: " + username + "\n");
        }
    }
    send(desc, &reply[0], reply.length(), 0);
}
...
void Tracker::list_files(string group_name, int desc) {
    ...
    if (allGroups.find(group_name) == allGroups.end()) {
        ...
        unordered_map<string, fileInfo> file = allGroups[group_name].files;
        bool flag = true;
        for (auto it : file) {
            if (flag) {
                reply += KGRN + it.first + RESET;
                flag = false;
            }
            else {
                reply += "\n" KGRN + it.first + RESET;
...
void Tracker::download_file(string group_name, string file_name, string destination, string username, int desc) {
    ...
    // }
        ...
        if (allGroups[group_name].files.find(file_name) == allGroups[group_name].files.end()) {
            ...
            fileInfo reqFile = allGroups[group_name].files[file_name];
            ...
            reply += " " + to_string(allGroups[group_name].files[file_name].noOfChunks) + " " + to_string(allGroups[group_name].files[file_name].lastChunkSize);
            reply += " " + allGroups[group_name].files[file_name].hash;
...
void Tracker::chunk_info(string group_name, string file_name, string username, int desc) {
    string reply;
    fileInfo reqFile = allGroups[group_name].files[file_name];
    reply += "$";
    bool flag = true;
