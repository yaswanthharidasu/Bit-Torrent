...
void Peer::download_file(string hash, int noOfChunks, long long lastChunkSize, string group_name, string file_name, vector<string> users, string destination) {
    ...
    // Now download each chunk by creating thread for each chunk
    // for (int i = 0; i < fileData.size(); i++) {
    ...
    // Create only 50 threads at max any time
...
void Peer::download_chunk(int chunk_no, long long chunk_size, vector<string>& chunkData, string file_name, string destination, string group_name) {
    ...
    while (!downloaded) {
        ...
        while (chunkData.size() == 0 || (tries == 2 * chunkData.size()) || first == true) {
            // Communicate with the tracker to get the chunk info
            log.printLog("File " + file_name + " | Waiting for others to upload \n");
            string message = GET_CHUNK_INFO " " + group_name + " " + file_name + " " + username;
